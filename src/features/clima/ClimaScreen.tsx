...
type OpenMeteoResponse = {
  current?: { temperature_2m?: number; wind_speed_10m?: number; precipitation_probability?: number };
...
async function fetchForecast(latitude: number, longitude: number): Promise<SimpleWeatherForecast> {
  ...
  if (!response.ok) throw new Error(`Falha ao buscar previsão (HTTP ${response.status})`);
...
export function ClimaScreen() {
  ...
  return (
    <Screen>
      <ScreenHeader kicker="Previsão simples" title="Clima" subtitle="Leitura rápida via GPS do celular" accent={palette.alertOrange} />

      {forecast ? (
        <Card title={rainAlert ? '⚠ Alto risco de chuva' : 'Condição atual'} accent={rainAlert ? palette.alertOrange : palette.neonBlue}>
          <View style={styles.grid}>
            <Metric palette={palette} label="Temperatura" value={`${Math.round(forecast.temperatureCelsius)}°C`} />
            <Metric palette={palette} label="Chance de chuva" value={`${forecast.rainChancePercent}%`} />
            <Metric palette={palette} label="Vento" value={`${forecast.windSpeedKmh} km/h`} />
          </View>
        ...
        <EmptyState icon="weather-partly-cloudy" title="Sem leitura ainda" description="Toque em atualizar para buscar a previsão usando o GPS do celular." />
      )}

      <PrimaryButton label={loading ? 'Buscando...' : 'Atualizar previsão'} icon="refresh" onPress={refresh} loading={loading} />

      <Text style={styles.footNote}>Dados de Open-Meteo (gratuito, sem chave). Última leitura fica salva no aparelho para consulta offline.</Text>
    </Screen>
  );
}

function Metric({ palette, label, value }: { palette: AppPalette; label: string; value: string }) {
  return (
    <View style={{ backgroundColor: palette.surfaceRaised, borderRadius: 12, flex: 1, padding: spacing.md }}>
      <Text style={{ color: palette.textMuted, fontSize: typography.caption, fontWeight: '800', letterSpacing: 0.5, marginBottom: spacing.xs, textTransform: 'uppercase' }}>{label}</Text>
      <Text style={{ color: palette.textPrimary, fontSize: typography.sectionTitle, fontWeight: '900' }}>{value}</Text>
    </View>
  );
}
...
const createStyles = (palette: AppPalette) =>
  StyleSheet.create({
    grid: { flexDirection: 'row', gap: spacing.sm, justifyContent: 'space-between', marginBottom: spacing.md },
    meta: { color: palette.textSecondary, fontSize: typography.caption, fontWeight: '700', marginBottom: spacing.xs },
